...
{
    vector<int> testSizes = { 256,512,1024,2048,4096,8192,16384,32768,65536,131072,262144,524288,1048576 };
    auto getTrials = [](int sz) -> int { if (sz<=8192) return 10; if (sz<=131072) return 5; return 3; };
    ...
    csv << "Size,Algorithm,AvgWallTime(s),AvgCpuTime(s),CPUUtil(%),BucketImbalance,BucketStdDev,NumTrials,NumChunks" << endl;

    cout << "============================================================" << endl;
    cout << "     SAMPLE SORT PERFORMANCE TEST  (Seq | A | B | C)"       << endl;
    cout << "============================================================" << endl;
    ...
    for (int size : testSizes) {
        ...
        csv << size << ",Sequential," << scientific << setprecision(10) << seqAvg << "," << seqAvg << ",100.00,1.0000,0.00," << nt << "," << numChunks << endl;

        const char* names[3] = { "SampleSortVersionA","SampleSortVersionB","SampleSortVersionC" };
        SortStats   vs[3]    = {};
        double      va[3]    = {};
        ...
        for (int v = 0; v < 3; v++) {
            ...
            cout << endl;
            csv << size << "," << names[v] << "," << scientific << setprecision(10) << vs[v].wallTimeSeconds << "," << vs[v].cpuTimeSeconds << "," << fixed << setprecision(2) << cu << "," << fixed << setprecision(4) << vs[v].bucketStats.imbalanceRatio() << "," << fixed << setprecision(2) << vs[v].bucketStats.stdDev() << "," << nt << "," << numChunks << endl;
        }
        cout << endl;
    }
